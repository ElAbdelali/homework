...
def repeat_string(phrase, num_of_times):
    print(phrase * num_of_times)
...
def num_spaces(seperate_this_sentence):
    counter = 0
    
    for i in range(len(seperate_this_sentence)):
        if seperate_this_sentence[i] == ' ':
            counter+=1
...
def write_letter(recipient_name, job_title, sender_name):
    return print(f"Dear {job_title} {recipient_name}, I think you are amazing! Sincerely, {sender_name}")
